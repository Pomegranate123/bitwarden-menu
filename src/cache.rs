...
pub fn read_ids() -> Option<Vec<String>> {
    ...
    if Path::new(&format!("{}/bw-rofi/cache.txt", cache_path)).exists() {
        let cache: Vec<String> = serde_json::from_str(
            &fs::read_to_string(format!("{}/bw-rofi/cache.txt", cache_path))
...
pub fn write(logins: &[Login]) {
    ...
    fs::create_dir_all(format!("{}/bw-rofi/images", cache_path)).unwrap();

    let mut rofi_cache = fs::File::create(format!("{}/bw-rofi/rofi.txt", cache_path)).unwrap();
    for login in logins {
        cache_image(login);
        write!(
            rofi_cache,
            "{}",
            format!(
                "{}\n<i>{}</i>\0icon\x1f{}/bw-rofi/images/{}.png\x0f",
    ...
    let mut buffer = fs::File::create(format!("{}/bw-rofi/cache.txt", cache_path)).unwrap();
...
fn cache_image(login: &Login) {
    ...
    if Path::new(&format!("{}/bw-rofi/images/{}.png", cache_path, login.id)).exists() {
        return;
    }
    match &login.uri {
        Some(website) => {
            ...
            let mut file =
                fs::File::create(format!("{}/bw-rofi/images/{}.png", cache_path, login.id))
                    .unwrap();
        ...
        None => std::os::unix::fs::symlink(
            format!("{}/bw-rofi/default.png", data_path),
            format!("{}/bw-rofi/images/{}.png", cache_path, login.id),
        )
        .unwrap(),
    }
}
